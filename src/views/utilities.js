...
function isKnownTypeCB(type){
  const ret= knownTypes.find(function matchingDishCB(t){
    return t === type;
  })
  return ret? true : false
    // you can use knownTypes.indexOf(TODO) and check if it's not negative. Don't forget the return keyword.
    // OR
    ...
    // once this function is written, a test should pass! Use the tests to check your work.
}
...
function dishType(dish){
    ...
    return "";
    // once this function is completed, more tests should pass! Use the tests to check your work.
}
...
*/
function dishComparatorCB(dishA, dishB){
    const typeA= dishType(dishA);
    const typeB= dishType(dishB);
    return knownTypes.indexOf(typeA)-knownTypes.indexOf(typeB);
    // use dishType(dishA) and dishType(dishB)
    // use Array.indexOf() to get the index of the type within `knownTypes`
...
function sortDishes(dishes){
    return [...dishes].sort(dishComparatorCB);
    // use someArray.sort(comparator_above_CB)
    // check the tests, look for sortDishes!
}

function ingredientComparatorCB(ingredientA, ingredientB){
...
function sortIngredients(ingredients){
  const ret = [...ingredients].sort(ingredientComparatorCB);
  return ret;
}
...
function menuPrice(dishesArray){
    ...
    const prices= dishesArray.map(dishToPriceCB);
    // 2) on the array of prices, call reduce() with the second calback as first parameter, and 0 as second parameter (we compute the total starting from zero).
    const sum= prices.reduce(sumCB, 0);
